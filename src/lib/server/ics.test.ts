...
beforeEach(() => {
	client.exec(
		'DELETE FROM bookings; DELETE FROM reminder_rules; DELETE FROM sessions; DELETE FROM participants; DELETE FROM experiments;'
	);
});
...
function seedBooking(id: string, sessionId: string, status = 'confirmed') {
	...
	client
		.prepare(
			'INSERT INTO bookings (id, session_id, participant_id, snapshot_name, snapshot_email, status, manage_token_hash) VALUES (?, ?, ?, ?, ?, ?, ?)'
		)
...
function seedRule(
	...
	client
		.prepare(
			'INSERT INTO reminder_rules (id, experiment_id, label, offset_minutes_before, condition, duration_minutes) VALUES (?, ?, ?, ?, ?, ?)'
		)
